...
final class ReviewNotice {
	...
	const OPTION     = 'advanced_testimonial_review';
	const REVIEW_URL = 'https://wordpress.org/support/plugin/advanced-testimonial/reviews/#new-post';
	...
	public function footer_text( $text ) {
		...
		return sprintf(
			...
			esc_html__( 'Enjoying Advanced Testimonial? Leave us a %1$s&#9733;&#9733;&#9733;&#9733;&#9733; review%2$s — it keeps development going.', 'advanced-testimonial' ),
			$link,
			'</a>'
		);
	}
	 ...
	 *
	 * Clicking "Rate it" only records that the review page was opened — the
	 * plugin cannot see WordPress.org reviews, so the prompts stop for good
	 * only after the user confirms "Yes, I left a review".
	 *
	 * @return void
	 */
	public function handle_actions() {
		...
		if ( empty( $_GET['at_review'] ) || ! current_user_can( 'manage_options' ) ) {
			return;
		}
		check_admin_referer( 'at_review_notice' );

		$state  = (array) get_option( self::OPTION, array() );
		$action = sanitize_key( wp_unslash( $_GET['at_review'] ) );
		...
		wp_safe_redirect( remove_query_arg( array( 'at_review', '_wpnonce' ) ) );
		exit;
	}
	...
	public function maybe_render() {
		...
		$state = (array) get_option( self::OPTION, array() );
		$later = wp_nonce_url( add_query_arg( 'at_review', 'later' ), 'at_review_notice' );
		$rate  = wp_nonce_url( add_query_arg( 'at_review', 'rate' ), 'at_review_notice' );
		$rated = wp_nonce_url( add_query_arg( 'at_review', 'rated' ), 'at_review_notice' );

		if ( empty( $state['asked'] ) ) {
			$message = '<strong>' . esc_html__( 'Enjoying Advanced Testimonial?', 'advanced-testimonial' ) . '</strong> '
				. esc_html__( 'A quick 5-star review helps other people find the plugin and keeps development going. Thank you!', 'advanced-testimonial' );
		} else {
			$message = '<strong>' . esc_html__( 'Did you get a chance to leave that review?', 'advanced-testimonial' ) . '</strong> '
				. esc_html__( 'If you did — thank you! Confirm below and we will never ask again.', 'advanced-testimonial' );
		}
		?>
			...
			<p style="margin:0;white-space:nowrap;">
				<?php if ( ! empty( $state['asked'] ) ) : ?>
					<a class="button" href="<?php echo esc_url( $rated ); ?>" style="margin-right:6px;">
						<?php esc_html_e( 'Yes, I left a review', 'advanced-testimonial' ); ?>
					</a>
				<?php endif; ?>
				<a class="button" href="<?php echo esc_url( $later ); ?>" style="margin-right:6px;">
					<?php esc_html_e( 'Maybe later', 'advanced-testimonial' ); ?>
				</a>
				<a class="button button-primary" href="<?php echo esc_url( self::REVIEW_URL ); ?>" target="_blank" rel="noopener noreferrer" onclick="window.location='<?php echo esc_js( $rate ); ?>';return true;">
					<?php esc_html_e( 'Rate it ★★★★★', 'advanced-testimonial' ); ?>
				</a>
			</p>
		</div>
		<?php
	...
	private function should_show() {
		...
		// Only ask people who actually use the plugin: at least one published
		// testimonial.
	...
	/**
	 * Whether the current admin screen belongs to this plugin — the post list,
	 * the editor, the taxonomy screen or any of its submenu pages, which all
	 * hang off edit.php?post_type=testimonial.
	 *
	 * @return bool
	 */
	private function on_own_screen() {
		...
		return $screen && CPT::POST_TYPE === $screen->post_type;
	}
}
